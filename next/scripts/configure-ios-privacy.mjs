...
for (const file of [privacySource, projectPath]) {
  await stat(file).catch(() => {
    throw new Error(`Falta ${path.relative(root, file)}. Ejecuta primero npm run cap:add:ios.`);
  });
}
...
const fileRef = 'FCN000010000000000000001';
const buildRef = 'FCN000010000000000000002';
...
const addAfter = (source, anchor, addition, description) => {
  ...
  if (!source.includes(anchor)) throw new Error(`No se encontró ${description} en project.pbxproj.`);
...
if (!project.includes(groupLine.trim())) {
  const appGroup = /(\/\* App \*\/ = \{\n\t\t\tisa = PBXGroup;\n\t\t\tchildren = \(\n)/;
  if (!appGroup.test(project)) throw new Error('No se encontró el grupo App en project.pbxproj.');
  project = project.replace(appGroup, `$1${groupLine}`);
}

if (!project.includes(resourceLine.trim())) {
  const resources = /(\/\* Resources \*\/ = \{\n\t\t\tisa = PBXResourcesBuildPhase;\n\t\t\tbuildActionMask = 2147483647;\n\t\t\tfiles = \(\n)/;
  if (!resources.test(project)) throw new Error('No se encontró la fase Resources en project.pbxproj.');
  project = project.replace(resources, `$1${resourceLine}`);
}

await writeFile(projectPath, project, 'utf8');
console.log('PrivacyInfo.xcprivacy instalado en FitCoach Next iOS.');
