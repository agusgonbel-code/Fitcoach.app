import type { Experience, UserProfile } from '../models';
...
function workout(profile: UserProfile, id: string, title: string, patterns: GeneratedExercise['pattern'][]): GeneratedWorkout {
  ...
  const exercises = picked.slice(0, maxExercises).map(item => ({ ...item, sets: profile.experience === 'beginner' ? Math.min(item.sets, 2) : item.sets }));
