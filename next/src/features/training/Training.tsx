...
function initialState(workout: GeneratedWorkout) {
  ...
  return {
    values: recovered?.values ?? blankDrafts(workout),
...
export function Training({ workout, sessions, onSaved }: { workout: GeneratedWorkout; sessions: WorkoutSession[]; onSaved: () => void }) {
  ...
  const summaries = useMemo(() => Object.fromEntries(workout.exercises.map((exercise) => [
    exercise.id,
    summarizeExerciseHistory({ sessions, exerciseId: exercise.id, repsMin: exercise.repsMin, repsMax: exercise.repsMax }),
  ])), [sessions, workout]);
  ...
  const update = (exerciseId: string, setIndex: number, key: keyof DraftSet, value: string) => setValues(current => ({
    ...current,
  ...
  const removeSet = (exerciseId: string, setIndex: number) => setValues(current => ({
    ...current,
    [exerciseId]: current[exerciseId].length <= 1 ? current[exerciseId] : current[exerciseId].filter((_, index) => index !== setIndex),
  }));
  ...
  const completeSet = (exercise: GeneratedExercise, setIndex: number) => {
    ...
    if (!set || set.reps === '' || !Number.isFinite(Number(set.reps)) || Number(set.reps) <= 0) {
      setError('Introduce las repeticiones de la serie antes de completarla.'); return;
    }
  ...
  const finish = () => {
    ...
    const session: WorkoutSession = {
      ...
      exercises: workout.exercises.map(exercise => ({
        ...
        sets: values[exercise.id].filter(set => set.reps !== '').map(set => ({ kg: Number(set.kg || 0), reps: Number(set.reps), rir: set.rir === '' ? null : Number(set.rir), completedAt: now })),
      })),
    };
  ...
  return <section>
    ...
    {workout.exercises.map(exercise => {
      ...
      return <article className="exercise-card" key={exercise.id}>
        ...
        <div className="coach-inline" role="note"><strong>Siguiente objetivo</strong><span>{summary.recommendation.message}</span></div>
