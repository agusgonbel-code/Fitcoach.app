...
function Onboarding({ onComplete }: { onComplete: (profile: UserProfile) => void }) {
  ...
  const changeTrainingDays = (value: number) => {
    ...
    setDays(normalized);
    setPreferredDays(defaultTrainingDays(normalized));
  };
...
function AuthenticatedApp({ profile }: { profile: UserProfile }) {
  ...
  const completedToday = Boolean(todayWorkout && sessions.some(session => session.localDate === localDate() && session.plannedWorkoutId === todayWorkout.id && Boolean(session.completedAt)));
