(function(root,factory){
  ...
  function allergyTerms(profile={}){return terms(profile.allergies);}
  function dislikeTerms(profile={}){return terms(profile.dislikes);}
  ...
  function filterPreferenceUnsafeRecipes(profile,recipes=[],ingredients=[]){
    const allergies=allergyTerms(profile),dislikes=dislikeTerms(profile);if(!allergies.length&&!dislikes.length)return recipes.slice();
    const map=Object.fromEntries((ingredients||[]).map(item=>[item.id,item]));
    return (recipes||[]).filter(recipe=>{
      if(allergies.length&&hasUnknownIngredients(recipe,map))return false;
      const text=recipeText(recipe,map);
      return !allergies.some(term=>text.includes(term))&&!dislikes.some(term=>text.includes(term));
    });
  }
